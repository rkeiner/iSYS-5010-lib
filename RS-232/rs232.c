...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode)
{
	...
	strcpy_s(mode_str, strlen(baud_s), baud_s);
	...
	strcat_s(mode_str, strlen(data_s), data_s);
	...
	strcat_s(mode_str, strlen(parity_s), parity_s);
	...
	strcat_s(mode_str, strlen(stop_s), stop_s);

	char *dtrrts_s = " dtr=on rts=on";
	strcat_s(mode_str, strlen(dtrrts_s), dtrrts_s);
...
int RS232_GetPortnr(const char *devname)
{
	...
	strncat_s(str, strlen(devname), devname, sizeof(str));
	str[31] = 0;
